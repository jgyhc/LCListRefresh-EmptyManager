...
@interface LCListRefreshManager ()<JCEmptyPageViewdelegate>
...
- (void)loadDataWithCompletion:(void (^)(NSError *error))completion isReload:(BOOL)isReload {
    ...
    if ([self.delegate respondsToSelector:@selector(jc_listRefreshManager:loadDataWithPageNo:pageCount:completion:)]) {
        [self beginLoading];
        __weak typeof(self) wself = self;
        [self.delegate jc_listRefreshManager:self loadDataWithPageNo:isReload?1:self.pageIndex pageCount:self.pageSize completion:^(NSArray *datas, NSError *error) {
            if (isReload) {//是否重新加载
                [self.tableView.mj_header endRefreshing];
...
- (void)beginLoading {
    self.isLoading = YES;
    if ([self.delegate respondsToSelector:@selector(jc_listRefreshManagerBeginLoading:)]) {
        [self.delegate jc_listRefreshManagerBeginLoading:self];
    }
}

- (void)endLoading {
    self.isLoading = NO;
    if ([self.delegate respondsToSelector:@selector(jc_listRefreshManagerEndLoading:)]) {
        [self.delegate jc_listRefreshManagerEndLoading:self];
    }
}

- (void)loadFailure:(NSError *)error {
    if ([self.delegate respondsToSelector:@selector(jc_listRefreshManagerFailure:error:)]) {
        [self.delegate jc_listRefreshManagerFailure:self error:error];
    }
}

#pragma mark - Getter & Setter

- (void)setStatus:(JCEmptyPageViewStatus)status {
    _status = status;
    self.emptyPageView.status = status;
}

- (JCEmptyPageView *)emptyPageView {
    if (!_emptyPageView) {
        _emptyPageView = [[JCEmptyPageView alloc] init];
